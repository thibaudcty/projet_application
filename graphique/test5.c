...
void button_clicked2(GtkWidget *widget, gpointer data){
	...
	const char * command = "echo yallah; screen -S tataLaScreen -X echo wazaaaa " ; 
	    int cr = system( command );
	    if ( cr != 0 ) {
		fprintf( stderr, 
			 "Impossible de lancer la commande : %s\n",
        ...
        const char * command2 = "screen -r ScreenScene; echo coucou; ./page_action; exit " ; 
	    int cr2 = system( command2 );
	    if ( cr2 != 0 ) {
		fprintf( stderr, 
			 "Impossible de lancer la commande 2 : %s\n",
			 command );
			}
		mainbis();	
